...
Machine::Machine(int window_length, QString path, StrMapReference ref, QObject *parent) :
    QObject(parent)
{
    ...
    feats = new Feats;
    _model = new model;
    vfeats = new QList<featuresOfSyllabel>;
    vfeats->clear();
...
size_t Machine::getByteOfUTF8(unsigned char c)
{
    if (c <= 0x7F) {
        ...
        return 0;    // c is not head byte of character
    }
}

/* Convert a string to vector of featuresOfSyllabel */
void Machine::convert(QString sentence)
{
    ...
    while (sentence[0] <= 40 && sentence.length() > 0) {
        sentence.erase(0);
    }

    size_t length = sentence.length();

    while (sentence[length - 1] <= 40 && length > 0) {
...
void Machine::extract(QString sentence, StrMapReference ref)
{
    ...
    for (int i = WINDOW_LENGTH; i < length - WINDOW_LENGTH - 1; ++i) {
        QSet<size_t> *featset = new QSet<size_t>;
        featset->clear();
...
QString Machine::segment(QString sentence)
{
    // initialize feats
    feats->clear();
